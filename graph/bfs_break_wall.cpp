...
bool bfs(int sX, int sY, int eX, int eY, int **a, int n, int **&ans) {
    ...
    while (!q.is_empty()) {
        ...
        for (int i = 0; i < 4; i ++) {
            int nx = x + dx[i];
            int ny = y + dy[i];
            if (nx >= 0 && nx < n && ny >= 0 && ny < n  && (f[nx][ny] == -1 || (f[nx][ny] != -1 && f[nx][ny] > val + 1 + a[nx][ny]))) {
                q.push(nx, ny, val + 1 + a[nx][ny]);
                f[nx][ny] = val + 1 + a[nx][ny];
            }
        }
    }
    cout << "nothing found\n";
...
void trace(int sX, int sY, int eX, int eY, int n, int** ans, int** a) {
    ...
    while (eX != sX || eY != sY) {
        ...
        for (int i = 0; i < 4; i ++) {
            int nx = eX + dx[i];
            int ny = eY + dy[i];
            if (nx >= 0 && nx < n && ny >= 0 && ny < n && ans[nx][ny] == ans[eX][eY] - a[eX][eY] - 1) {
                eX = nx;
                eY = ny;
                path[nx][ny] = true;
                break;
